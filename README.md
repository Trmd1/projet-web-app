L3 MN <br>
RAYMOND Théo <br>
VANDERHOEVEN LEROY ARMAND<br>
<br>
Ce projet a pour but la réalisation d'une API via FastApi. <br>
Nous avons pris pour sujet principal de cette API le monde des Pokémons ET celui de Minecraft. <br>
En effet, cette api nous permettra d'accéder aux pokémons et leur donner inscrites et disponibles dans le mod Pixelmon.
<br>
<br>
Afin de lancer le projet pour la première fois sur une machine il faut run docker compose up dans un terminal à l'emplacement de la copie à la suite de celui pour relancer le projet il suffit de run docker compose up fastapi au même endroit
<hr>

Tooltip : 
<br>
docker compose up (pour la mise en route)
<br>
localhost:8000 -> port d'accès serveur de l'api
<br>
localhost:8000/docs (pour la doc générée par FastAPI)
<br>


<img src="https://cdn.modrinth.com/user/yDIJMMci/120105c19ab989157820575680d7ed58f6139ad1.png", width='200px'>
<br>

